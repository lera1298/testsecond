...
static int strlen_rom(const char *s);
static int isdigit_rom(int c);
static int tolower_rom(int c);
...
/* void esp_printf( const func_ptr f_ptr,
   const charptr ctrl1, ...) */
void ee_printf( const charptr ctrl1, ...)
{
...
int isdigit_rom(int c)
{
        return (c >= '0' && c <= '9');
}

int tolower_rom(int c)
{
        if(c >= 'A' && c <= 'Z') {
                c = c + ('a' - 'A');
        }
