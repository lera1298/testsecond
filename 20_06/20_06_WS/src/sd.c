...
int FfsSdPolledExample(void)
{
	...
	xil_printf("f_mount done\r\n");
	/*
	 * Path - Path to logical driver, 0 - FDISK format.
	 * 0 - Cluster size is automatically determined based on Vol size.
	 */
	 Res = f_mkfs(Path, 0, workBuf, 1024);
	 if (Res != FR_OK) {
		 xil_printf("Res f_mkfs: %d\r\n", Res);
	 	return XST_FAILURE;
	 }
	 xil_printf("f_mkfs done\r\n");
	/*
	...
	if (Res) {
		xil_printf("Res f_open: %d\r\n", Res);
		return XST_FAILURE;
	}
	xil_printf("f_open done\r\n");
	/*
...
void init_sd_core()
{
	...
	XLlFifo_Initialize(&in_fifo, XPAR_AXI_FIFO_MM_S_0_BASEADDR);
	XLlFifo_Initialize(&out_fifo, XPAR_AXI_FIFO_MM_S_1_BASEADDR);
}
...
//	taskENTER_CRITICAL();
	...
	xil_printf("number of sectors: %u\r\n", (sizeSD + 1) * 1024);
//	taskEXIT_CRITICAL();
