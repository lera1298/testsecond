...
int generate_http_header(char *buf, char *fext, int fsize)
{
	...
	else if (!strncmp(fext, "json", 3))
		strcat(buf, "application/json");
	else if (!strncmp(fext, "js", 2))
		strcat(buf, "application/javascript");
	else if (!strncmp(fext, "wasm", 2))
			strcat(buf, "application/wasm");
	else if (!strncmp(fext, "svg", 2))
			strcat(buf, "image/svg+xml");
	else if (!strncmp(fext, " ico", 2))
				strcat(buf, "text/htm");
	else if (!strncmp(fext, "svg", 2))
				strcat(buf, "application/wasm");
